...
namespace Gaia::UDP
{
     ...
     *  Receiver can only begin to receive data only after it's bound to a specific port by invoking Bind(...) function.
     */
	class Receiver
	{
	private:
	...
	public:
		 ...
		 * @details
		 *  After this function is called, incoming data will be stored in the buffer,
		 *  which means transmitted data before this function call will not be received.
		 */
		std::tuple<Endpoint, ByteUtility::BytesBuffer> Receive();
		/**
